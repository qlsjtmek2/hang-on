...
if [[ ${#services_with_changes[@]} -gt 0 ]]; then
    ...
    echo "Changes detected in: $services_list — triggering auto-error-resolver..." >> /tmp/claude-hook-debug.log
...
EOF
        ...
        if [ $? -ne 0 ]; then
            ...
            claude chat "Use the auto-error-resolver agent to build and fix errors in: ${services_list}" 2>> /tmp/claude-hook-debug.log
        fi
    else
