...
describe('RecordCard Component', () => {
  const mockRecord = {
    ...
    content:
      '오늘은 날씨가 흐려서 기분도 약간 가라앉은 느낌이다. 하지만 따뜻한 차를 마시며 조금씩 기분이 나아지고 있다.',
    createdAt: new Date(),
    empathyCount: 5,
    messageCount: 3,
  };
  ...
  it('renders with correct emotion data', () => {
    const { getByText } = render(<RecordCard {...mockRecord} />);
  ...
  it('truncates long content', () => {
    ...
    const { getByText } = render(<RecordCard {...mockRecord} content={longContent} />);
  ...
  it('hides counts when they are zero', () => {
    const { queryByText } = render(
      <RecordCard {...mockRecord} empathyCount={0} messageCount={0} />,
    );
  ...
  it('calls onPress when card is pressed', () => {
    ...
    const { getByRole } = render(<RecordCard {...mockRecord} onPress={onPress} />);
  ...
  it('calls onEmpathyPress when empathy button is pressed', () => {
    ...
    const { getByText } = render(<RecordCard {...mockRecord} onEmpathyPress={onEmpathyPress} />);
  ...
  it('calls onMessagePress when message button is pressed', () => {
    ...
    const { getByText } = render(<RecordCard {...mockRecord} onMessagePress={onMessagePress} />);
  ...
  it('shows shared badge when isShared is true', () => {
    const { getByText } = render(<RecordCard {...mockRecord} isShared={true} />);

    expect(getByText('공유됨')).toBeTruthy();
  });

  it('does not show shared badge when isShared is false', () => {
    const { queryByText } = render(<RecordCard {...mockRecord} isShared={false} />);
  ...
  it('displays correct emotion for each level', () => {
    ...
    levels.forEach((level, index) => {
      const { getByText } = render(<RecordCard {...mockRecord} emotionLevel={level} />);
  ...
  it('uses custom time formatter when provided', () => {
    ...
    const { getByText } = render(<RecordCard {...mockRecord} formatTime={customFormatter} />);
  ...
  it('handles string date format', () => {
    ...
    const { getByText } = render(<RecordCard {...mockRecord} createdAt={dateString} />);
  ...
  it('has correct accessibility properties', () => {
    const { getByRole } = render(<RecordCard {...mockRecord} />);
  ...
  it('applies custom styles', () => {
    ...
    const { getByRole } = render(<RecordCard {...mockRecord} style={customStyle} />);
